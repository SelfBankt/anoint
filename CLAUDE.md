...
`brand/anoint-mark-white.png` is the hero's centered wordmark, derived from `brand/anoint-mark-
source.png` (the original black-on-transparent artwork, kept in the repo for reproducibility —
black is unusable as-is against the hero's `--ink` background). Generated by keeping the source's
alpha channel exactly as-is and replacing RGB with flat white
(confirmed via pixel inspection first that the source is a true monochrome mark — solid near-black
RGB with alpha-only anti-aliasing at the edges, not a color gradient — so a flat recolor is correct
and doesn't need a naive per-channel invert), then cropped to the artwork's bounding box (the
source PNG has a lot of transparent padding around it). If the logo ever needs updating, redo both
steps from a fresh source file rather than hand-editing this derived one. It replaces what was
previously a plain text eyebrow ("ANOINT · GENESIS SCENT") above the hero headline — don't add that
text back alongside the logo, it says the same thing twice.
